...
class ExpDesignAccess:
    ...
    def read_expdesign_file(self, run):
        """
        ...
        expdesign_file = (
            ...
            / f"experiment_settings_sub_{self.sub:02d}_ses_{self.ses:02d}_run_{run:02d}.yml"
        )
    ...
    def append_all_trails_without_blanks(self):
        session_trails_without_blanks = []
        for run in range(1, self.run_number + 1):
            expdesign = self.read_expdesign_file(run)
